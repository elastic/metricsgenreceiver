...
* `seed_randomizer_enabled` (default `false`): when set to true, the seed value will be used to generate deterministic but seed-dependent host names in the format `host-{seed_prefix}-{id}`. This allows different seeds to produce different host names while maintaining determinism. When false (default), the original host naming format is used (e.g., `host-{id}`, `nginx-host-{id}`, etc.) for backward compatibility.
